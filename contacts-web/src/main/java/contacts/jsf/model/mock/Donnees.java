...
public class Donnees implements Serializable {
	...
	private void initialiserDonnees() {
		...
		Compte compte;
		compte = new Compte( 1, "geek", "geek", "geek@3il.fr", true );
		mapComptes.put( compte.getId(), compte );

		compte = new Compte(2, "chef", "chef", "chef@3il.fr", false);
		mapComptes.put( compte.getId(), compte );
		
		compte = new Compte( 3, "job", "job", "job@3il.fr", false );
		mapComptes.put( compte.getId(), compte );

		compte = new Compte(4, "_" + this.getClass().getPackage().getName(), "xxx", "xxx@3il.fr", false);
		mapComptes.put( compte.getId(), compte );
